...
<body class="d-flex h-100 text-center text-white bg-dark ">
    ...
    <div class="cover-container d-flex w-100 h-100 p-3 mx-auto flex-column">
        ...
        <form action="recherchefilmtitre.php" method="post">
            <input type="text" name="keywords" placeholder="search by titles">
        ...
        <form action="recherchefilmgenre.php" method="post">
            <select name="type">
                ...
                $genres = $bdd->query("SELECT * FROM genre");
        ...
        <strong>Search Film By Distributor</strong>
        <form action="recherchefilmdistri.php" method="post">
            <select name="type">
                <option value="NULL">Aucun</option>
                <?php
                $bdd = new PDO('mysql:host=localhost;dbname=cinema;charset=utf8', 'maroua', 'Studentes01@');
                $distris = $bdd->query("SELECT * FROM distributor");
                foreach ($distris as $distri) {
                    echo '<option value="' . $distri["name"] . '">' . $distri["name"] . '</option>';
                }
                ?>
            </select>
