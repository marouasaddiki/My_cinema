...
function searchid()
{
  ...
  {
    $like.= "lastname LIKE '%".$keyword."%' OR firstname LIKE '%".$keyword."%' OR";
  }
    $like = substr($like, 0, strlen($like) - 3);
    $membres = $bdd->query("SELECT * FROM user WHERE ".$like);
      while ($data = $membres->fetch())
      {
        echo '<a href="membre.php?id='.$data["id"].'"><div class="membres">'.$data["lastname"] . ' ' . $data["firstname"].'</div><br/></a>';
      }
  return;
}
searchid();
